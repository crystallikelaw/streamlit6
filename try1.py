...
st.markdown('# Youtube for Gaming Content Creators')
...
if app_mode=='Dashboard':
    ...
    if filter == 'Full Dataset':
        ...
        allvids = st.selectbox("", ["Views", "Engagement", "Output", "Game Variety"])
        # , "Genres", "Games"
        if allvids == 'Views':
            st.markdown('#### Most Viewed Channels :')
            st.write('These are channels that have the most views, in total.')
            tviews = df_vids.groupby(by=['Channel Title']).Views.sum().sort_values(ascending=False).reset_index()
            # st.write(tviews)
            fig = plt.figure(figsize=(10, 5))
            plt.xticks(rotation=40, fontsize=8)
            ax = sns.barplot(tviews, x='Channel Title', y='Views', color='steelblue')
            ...
            tviews2 = df_vids.groupby(by=['Channel Title']).Views.sum()/df_vids.groupby(by=['Channel Title']).Views.count()
            tviews2 = tviews2.reset_index().sort_values(by=['Views'], ascending=False)
            # st.write(tviews2)
            fig = plt.figure(figsize=(10, 5))
            plt.xticks(rotation=40, fontsize=8)
            ax = sns.barplot(tviews2, x='Channel Title', y='Views', color='steelblue')
            xlabels = ['{:,.0f}'.format(x) + 'M' for x in ax.get_yticks()/1000000]
        ...
        if allvids == 'Engagement':
            ...
            tviews = df_vids.groupby(by=['Channel Title']).Engagement.sum().sort_values(ascending=False).reset_index()
            # st.write(tviews)
            fig = plt.figure(figsize=(10, 5))
            plt.xticks(rotation=40, fontsize=8)
            ax = sns.barplot(tviews, x='Channel Title', y='Engagement', color='steelblue')
            ...
            tviews2 = df_vids.groupby(by=['Channel Title']).Engagement.sum()/df_vids.groupby(by=['Channel Title']).Engagement.count()

            tviews2 = tviews2.reset_index().sort_values(by=['Engagement'], ascending=False)
            # st.write(tviews2)
            fig = plt.figure(figsize=(10, 5))
            plt.xticks(rotation=40, fontsize=8)
            ax = sns.barplot(tviews2, x='Channel Title', y='Engagement', color='steelblue')
            xlabels = ['{:,.0f}'.format(x) + 'K' for x in ax.get_yticks()/1000]
            ...
            tviews2 = df_vids.groupby(by=['Channel Title']).Engagement.sum()/df_vids.groupby(by=['Channel Title']).Views.sum()
        ...
        if allvids == 'Output':
            ...
            tviews = df_vids.groupby(by=['Channel Title']).Engagement.count().sort_values(ascending=False).reset_index()
            # st.write(tviews)
            fig = plt.figure(figsize=(10, 5))
            plt.xticks(rotation=40, fontsize=8)
            ax = sns.barplot(tviews, x='Channel Title', y='Engagement', color='steelblue')
        ...
        if allvids == 'Game Variety':
            ...
            tviews = tviews = df_vids.groupby(by=['Channel Title'])["Game Played"].nunique().sort_values(ascending=False).reset_index()
            # st.write(tviews)
            fig = plt.figure(figsize=(10, 5))
            plt.xticks(rotation=40, fontsize=8)
            ax = sns.barplot(tviews, x='Channel Title', y='Game Played', color='steelblue')
            # xlabels = ['{:,.0f}'.format(x) + 'K' for x in ax.get_yticks()/1000]
    ...
    if filter == 'Single Channel':
       ...
       'KatherineOfSky', 'SmallAnt'))
        ...
        allvids = st.selectbox("", ["Best Videos", "Distributions", "Over Time", "Best Games", "Correlation"])
        ydf = df_vids[df_vids['Channel Title'] == ytber]

        if allvids == 'Best Videos':
            st.write(ydf[['Video Title', 'Likes', 'Comments', 'Views', 'Game Played', 'Engagement']].sort_values(by=['Views'], ascending=False).head(20))
            # st.write(ydf.columns)


        if allvids == 'Distributions':
            var = st.radio('Select Variable',['Views','Likes', 'Comments', 'Engagement'])
            # st.write(ydf)
            fig = plt.figure(figsize=(10, 5))
        ...
        if allvids == 'Over Time':
            ...
            ax = sns.regplot(ydf, x='Upload Year', y='Views', color='steelblue', scatter=True)
            ...
            fig = plt.figure(figsize=(10, 5))
            plt.xticks(rotation=40, fontsize=8)
            ax =  sns.regplot(ydf, x='Upload Year', y='Engagement', color='steelblue', scatter=True)
            # ylabels = ['{:,.0f}'.format(x) + 'M' for x in ax.get_yticks()/1000000]
            ax.set(ylabel="Average Engagement")
            # ax.set_yticklabels(ylabels)
            st.pyplot(plt)
        if allvids == 'Best Games':
            tviews = pd.DataFrame(ydf.groupby(by=['Game Played']).Views.sum()/ ydf.groupby(by=['Game Played']).Views.count())
            tviews['Engagement'] = ydf.groupby(by=['Game Played']).Engagement.sum()/ ydf.groupby(by=['Game Played']).Views.count()
            tviews['Likes'] = ydf.groupby(by=['Game Played']).Likes.sum() / ydf.groupby(by=['Game Played']).Views.count()
            tviews['Comments'] = ydf.groupby(by=['Game Played']).Comments.sum()/ ydf.groupby(by=['Game Played']).Views.count()
            tviews = tviews.sort_values(by=['Views'], ascending=False).reset_index()
            tviews.rename(columns={'Views':"Average Views", "Likes": "Average Likes", "Comments":"Average Comments", "Engagement": "Average Engagement"}, inplace=True)
            st.write(tviews.head(11))
            fig = plt.figure(figsize=(10, 5))
            plt.xticks(rotation=60, fontsize=8)
            ax = sns.barplot(tviews.head(20), x='Game Played', y='Average Views', color='steelblue')
            xlabels = ['{:,.0f}'.format(x) + 'K' for x in ax.get_yticks()/1000]
            ax.set(ylabel="Average Views (in thousands)")
            ax.set_yticklabels(xlabels)
            st.pyplot(plt)
            tviews = tviews.sort_values(by=['Average Engagement'], ascending=False).reset_index()
            fig = plt.figure(figsize=(10, 5))
            plt.xticks(rotation=60, fontsize=8)
            ax = sns.barplot(tviews.head(20), x='Game Played', y='Average Engagement', color='steelblue')
            # xlabels = ['{:,.0f}'.format(x) + 'B' for x in ax.get_yticks()/1000000000]
            ax.set(ylabel="Average Engagement")
            # ax.set_yticklabels(xlabels)
            st.pyplot(plt)
...
if app_mode == 'Recommendation':
    ...
    def getRecs(titlelist: list[str], count: int = 10, yt=0) -> list:
        ...
        idxs = []
        if yt:
            titlelist = df_vids[df_vids['Channel Title'] == yt].groupby(by=['Game Played']).Views.sum().sort_values(ascending=False).head(10).index
