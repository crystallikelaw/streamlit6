...
def videoInfoScraper(dfWithVideoID: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        ...
        dfWithVideoID = dfWithVideoID.reindex(columns=dfWithVideoID.columns.tolist() + ['likes', 'comments', 'views', 'game'])
    try:
        driver.quit()
    except:
        pass
    ...
    for row in t:
        idx = row[0]
        if (dfWithVideoID.loc[idx, 'likes'] > 0) or (dfWithVideoID.loc[idx, 'comments'] > 0) or (dfWithVideoID.loc[idx, 'views'] > 0): # skipping rows for restart
            continue
        ...
        try:
            ...
            print("Timeout @", idx)
            dfWithVideoID.to_pickle('_videoInfoScrape.pkl') # saves at breakpoint
            driver.quit()
            sleep(10)
            failed = True
            break
